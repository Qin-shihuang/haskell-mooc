...
maxHelper v acc = if acc >= v then acc else v
...
largestHelper v acc = case acc of
    [] -> [v]
    (x:xs) -> if v > x then [v] else if v == x then v:acc else acc
