...
union s1 s2 = Shape g
  where
    g (Coord x y) = contains s1 x y || contains s2 x y

cut :: Shape -> Shape -> Shape
cut s1 s2 = Shape g
  where
    g (Coord x y) = contains s1 x y && not (contains s2 x y)
...
paintSolid color (Shape s) (Picture b) = Picture g
  where
    g coord = if s coord then color else b coord
...
paint (Picture pat) (Shape s) (Picture b) = Picture g
  where
    g coord = if s coord then pat coord else b coord
...
instance Transform Fill where
  apply f (Picture p) = Picture g
    where
      g _ = case f of Fill c -> c

data Zoom = Zoom Int
  deriving (Show)

instance Transform Zoom where
  apply z (Picture p) = Picture g
    where
      g = p . zoomCoord (case z of Zoom z -> z)
...
instance Transform Flip where
  apply f (Picture p) = case f of
    FlipX -> Picture (p . \(Coord x y) -> Coord (-x) y)
    FlipY -> Picture (p . \(Coord x y) -> Coord x (-y))
    FlipXY -> Picture (p . flipCoordXY)
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  ...
  apply c (Picture p) = case c of
    Chain t1 t2 -> apply t1 (apply t2 (Picture p))
...
instance Transform Blur where
  apply b (Picture p) = Picture g
    where
      g (Coord x y) = Color (avgRed x y) (avgGreen x y) (avgBlue x y)
      avgRed x y = div (sum [getRed (p (Coord x' y')) | (x', y') <- neighbours x y]) 5
      avgGreen x y = div (sum [getGreen (p (Coord x' y')) | (x', y') <- neighbours x y]) 5
      avgBlue x y = div (sum [getBlue (p (Coord x' y')) | (x', y') <- neighbours x y]) 5
      neighbours x y = [(x + dx, y + dy) | dx <- [-1, 0, 1], dy <- [-1, 0, 1], abs dx + abs dy <= 1]
