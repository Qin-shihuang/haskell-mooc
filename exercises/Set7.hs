...
add a (Set as) = if elem a as then Set as else Set (sort (a:as))
...
data State = Start | Error | Finished | Eggs | Flour | Sugar | Both | Mixed
  deriving (Eq,Show)
...
reverseNonEmpty (a :| as) = case reverse as of
  [] -> a :| []
  (b:bs) -> b :| (bs ++ [a])
...
passwordAllowed s pr = case pr of
  MinimumLength n -> length s >= n
  ContainsSome cs -> any (`elem` cs) s
  DoesNotContain cs -> not (any (`elem` cs) s)
