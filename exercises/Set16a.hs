...
isSorted xs = if null xs then property True else property $ and $ zipWith (<=) xs (tail xs)
...
sumIsLength input output = if length input == (sum $ map snd output) then property True else property False 
...
frequenciesProp freq input = conjoin [sumIsLength (getNonEmpty input) (freq $ getNonEmpty input),
