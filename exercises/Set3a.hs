...
multiCompose fs = multiComposeHelper fs
    where multiComposeHelper [] x = x
          multiComposeHelper fs x = multiComposeHelper (take (length fs - 1) fs) (last fs x)
------------------------------------------------------------------------------
...
interpreter commands = interpreterHelper commands 0 0
    where interpreterHelper [] x y = []
          interpreterHelper (command:commands) x y
            ...
            | command == "printY" = show y : interpreterHelper commands x y
            | otherwise = interpreterHelper commands x y
