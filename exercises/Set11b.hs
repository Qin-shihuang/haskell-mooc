...
appendAll r = mapM_ (\s -> modifyIORef r (++ s))
------------------------------------------------------------------------------
...
hSelectLines h = hFetchLines' h 1 where
    hFetchLines' h n [] = return []
    hFetchLines' h n (i:is) = do
        line <- hGetLine h
        if n == i then do
            rest <- hFetchLines' h (n+1) is
            return (line:rest)
        else do
            hFetchLines' h (n+1) (i:is)
