...
greetText name = if T.length name > 15 then T.concat [T.pack "Hello, ", T.take 15 name, T.pack "...!"] else T.concat [T.pack "Hello, ", name, T.pack "!"]
...
shout :: T.Text -> T.Text
shout t = T.unwords $ map (\(i, w) -> if odd i then T.toUpper w else w) $ zip [1..] $ T.words t
...
longestRepeat t = maximumOrZero $ map T.length $ T.group t
    where maximumOrZero [] = 0
...
pingpong = BL.cycle . BL.fromStrict . (\b -> B.concat [b, B.reverse b])
