...
longest :: Ord a => [[a]] -> [a]
longest xss = foldl (\acc xs -> if length xs > length acc || (length xs == length acc && head xs < head acc) then xs else acc) (head xss) xss
...
freqs xs = Map.fromList $ foldr (\x acc -> case lookup x acc of
    Just n -> (x, n + 1) : filter ((/= x) . fst) acc
    Nothing -> (x, 1) : acc) [] xs
...
transfer from to amount bank =  case (Map.lookup from bank, Map.lookup to bank) of
    (Just fromAccount, Just toAccount) -> if amount >= 0 && fromAccount >= amount 
        then Map.insert from (fromAccount - amount) $ Map.insert to (toAccount + amount) bank
        else bank
    _ -> bank
...
maxIndex arr = let arrIndices = indices arr 
    in foldl (\acc i -> if arr ! i > arr ! acc then i else acc) (head arrIndices) (tail arrIndices)
