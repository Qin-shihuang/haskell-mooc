...
sums :: Int -> [Int]
sums i = sumsHelper 1 1 i
  where
    sumsHelper :: Int -> Int -> Int -> [Int]
    sumsHelper sum c i =
      if c == i
        then [sum]
        else sum : sumsHelper (sum + c + 1) (c + 1) i
...
sumsOf xs = sumsOfHelper 0 xs
  where
    ...
    sumsOfHelper sum xs =
      case xs of
        []   -> []
        x:xs -> sum + x : sumsOfHelper (sum + x) xs
...
map2 :: (a -> b -> c) -> [a] -> [b] -> [c]
map2 f as bs = case as of
    [] -> []
    a:as -> case bs of
      [] -> []
      b:bs -> f a b : map2 f as bs
